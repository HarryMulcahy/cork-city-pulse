        ...
        {fetching ? (
          ...
          <ul className="space-y-3">
            {rows.map((r) => {
              ...
              return (
                <li key={r.id} className="rounded-lg border border-border bg-card overflow-hidden flex hover:border-primary/50 transition-colors">
                  <Link
                    to="/"
                    search={{ dev: r.id }}
                    className="flex flex-1 min-w-0"
                    aria-label={`Open ${r.title} on the map`}
                  >
                    {r.images[0] ? (
                    ...
                    <div className="flex-1 p-4 min-w-0">
                      ...
                      {tab === "review" && r.approval_status === "pending" && (
                        <div className="flex gap-2 mt-3" onClick={(e) => e.preventDefault()}>
                          <Button size="sm" disabled={acting === r.id} onClick={(e) => { e.preventDefault(); approve(r.id); }} className="gap-1.5">
